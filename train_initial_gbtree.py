...
def model_explain(model, data_to_predict):
    ...
    importance = pd.DataFrame(list(zip(X_train.columns, vals)), columns=['Feature', 'Feature Importance'])
    for ind in importance.index:
        if ind > 9:
            importance.loc[ind, 'Feature'] = "'" + importance.loc[ind, 'Feature'] + "'"
    ...
    # Make the plot
    plt.bar(r1, bars1, width=barWidth, edgecolor='white', label='Preoperative Features')
