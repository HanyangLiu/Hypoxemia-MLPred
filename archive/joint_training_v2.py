...
class JointDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        X_future = np.empty((len(list_IDs_temp), *self.dim))
        ...
        df_seed_cut.loc[list(df_seed_cut.groupby(['pid']).tail(5).index), df_seed_cut.columns[3:]] = 0
        for j in range(self.sliding_window):
            ...
            X_future[:, j, :] = slice_Y.iloc[len(df_seed) - len(list_IDs_temp):, 3:]

        if len(X_future) != len(X):
            print("Error!!! - Shapes of X and y do not match!")

        # y: (n_samples, 1)
        y = self.labels.loc[self.labels['index'].isin(list_IDs_temp), 'label'].values

        return X, [X, X_future, keras.utils.to_categorical(y, num_classes=2)]
...
def multitask_rnn(window_size, n_features):
    ...
    decoder1 = layers.Dense(128, activation='relu')(encoder)
    decoder1 = layers.RepeatVector(n_in)(decoder1)
    ...
    pred_hidden = layers.Dense(128, activation='relu', name='pred_hidden')(encoder)
    decoder2 = layers.RepeatVector(n_in)(pred_hidden)
    decoder2 = layers.LSTM(128, activation='relu', return_sequences=True)(decoder2)
    decoder2 = layers.TimeDistributed(Dense(n_features), name='decoder2_output')(decoder2)
    # define outcome predictor
    predictor = layers.Dense(128, activation='relu')(pred_hidden)
    ...
    model.compile(optimizer='adam', loss={'decoder1_output': 'mse',
                  ...
                  loss_weights={'decoder1_output': args.weight,
                                'decoder2_output': args.hypoxemia_window * args.weight,
                                'predictor_output': 1 - args.weight})
    # model.compile(optimizer='adam', loss='mse')
    try:
        keras.utils.plot_model(model, show_shapes=True, to_file='multitask_rnn_v2.png')
    except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="data/model/model_joint_v2.h5")
