...
from utils.utility_lstm_data import PredictorDataGenerator
...
class JointDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, timeSeries, static, labels_1, labels_2, batch_size=1024, window_size=10, horizon=5, shuffle=False):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # y: (n_samples, 1)
        labels_1 = self.labels_1.loc[self.labels_1['index'].isin(list_IDs_temp), 'label'].values
        labels_2 = self.labels_2.loc[self.labels_2['index'].isin(list_IDs_temp), 'label'].values

        return X, [X, y, keras.utils.to_categorical(labels_1, num_classes=2), keras.utils.to_categorical(labels_2, num_classes=2)]


class MultiPredictorDataGenerator(keras.utils.Sequence):
    'Generates data for Keras'
    def __init__(self, timeSeries, static, labels_1, labels_2, batch_size=1024, window_size=10, n_classes=2, shuffle=False):
        # "shuffle" cannot be set as True here !!
        'Initialization'
        self.n_features = len(timeSeries.columns) - 3
        self.batch_size = batch_size
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # y: (n_samples, 1)
        y_1 = self.labels.loc[self.labels_1['index'].isin(list_IDs_temp), 'label'].values
        y_2 = self.labels.loc[self.labels_2['index'].isin(list_IDs_temp), 'label'].values

        return X, [keras.utils.to_categorical(y_1, num_classes=self.n_classes), keras.utils.to_categorical(y_2, num_classes=self.n_classes)]


def prepare_data(df_static, df_dynamic, hypoxemia_window):
    '''Prepare Data'''
    ...
    path_sta_label = 'data/label/static_label_lstm_' + str(hypoxemia_window) + '.pkl'
    path_dyn_label = 'data/label/dynamic_label_lstm_' + str(hypoxemia_window) + '.pkl'
...
def multitask_rnn(window_size, n_features):
    ...
    # define outcome predictor1
    predictor = layers.LSTM(64, activation='relu')(pred_hidden)
    predictor1 = layers.Dense(2, activation='softmax', name='predictor_1')(predictor)
    # define outcome predictor2
    predictor2 = layers.Dense(2, activation='softmax', name='predictor_2')(predictor)

    # tie it together
    model = models.Model(inputs=visible, outputs=[decoder1, decoder2, predictor1, predictor2])
    model.summary()
    keras.utils.plot_model(model, show_shapes=True, to_file='multitask_rnn_v4.png')

    model.compile(optimizer='adam', loss={'decoder1_output': 'mse',
                                          'decoder2_output': 'mse',
                                          'predictor_1': 'categorical_crossentropy',
                                          'predictor_2': 'categorical_crossentropy'},
                  loss_weights={'decoder1_output': args.weight,
                                'decoder2_output': 1 - args.weight,
                                'predictor_1': 1 - args.weight,
                                'predictor_2': 1 - args.weight})
    # model.compile(optimizer='adam', loss='mse')

    model_predictor = models.Model(inputs=model.inputs, outputs=[predictor1, predictor2])
...
def train(timeSeriesTr, labelsTr_1, labelsTr_2, timeSeriesTe, labelsTe_1, labelsTe_2, args):

    batch_size = args.batch_size
    window_size = args.steps
    n_features = len(timeSeriesTr.columns) - 3
    train_data_generator = JointDataGenerator(timeSeriesTr, static_feat, labelsTr_1, labelsTr_2,
                                              batch_size=batch_size,
                                              window_size=window_size,
                                              horizon=args.horizon,
                                              shuffle=False)
    valid_data_generator = JointDataGenerator(timeSeriesTe, static_feat, labelsTe_1, labelsTe_2,
                                              batch_size=batch_size,
                                              window_size=window_size,
                                              horizon=args.horizon,
                                              shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="data/model/model_joint_v4.h5")
    ...
    timeSeriesTr, labelsTr_1, timeSeriesTe, labelsTe_1, pos_rate = prepare_data(df_static, df_dynamic,
                                                                                hypoxemia_window=1)
    _, labelsTr_2, _, labelsTe_2, _ = prepare_data(df_static, df_dynamic, hypoxemia_window=5)
    model, _ = train(timeSeriesTr, labelsTr_1, labelsTr_2, timeSeriesTe, labelsTe_1, labelsTe_2, args)
    model.save(args.model_path)
    joint_model = models.load_model(args.model_path)
    test_data_generator_1 = PredictorDataGenerator(timeSeriesTe, static_feat, labelsTe_1,
                                                   batch_size=len(labelsTe_1),
                                                   window_size=args.steps,
                                                   n_classes=2)
    test_data_generator_2 = PredictorDataGenerator(timeSeriesTe, static_feat, labelsTe_2,
                                                   batch_size=len(labelsTe_2),
                                                   window_size=args.steps,
                                                   n_classes=2)
    evaluate(joint_model, test_data_generator_1, test_data_generator_2, pos_rate)
