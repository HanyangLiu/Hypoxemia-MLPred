...
for win_size in [10, 10, 10, 10]:

    args.hypoxemia_prediction = win_size
    print(args)
    ...
    fpr, tpr, _ = metrics.roc_curve(y_test, y_prob2)
    prec, rec, _ = metrics.precision_recall_curve(y_test, y_prob2)
    (sensitivity, specificity, PPV, NPV, f1, acc), _ = line_search_best_metric(y_test, y_prob2, spec_thresh=0.95)
